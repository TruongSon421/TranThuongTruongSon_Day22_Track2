...
def main():
    ...
    ================================================
    V1 (concise, 2-4 sentences) scored higher on faithfulness (0.923 vs 0.918)
    and context recall (0.85 vs 0.83) for the following reasons:

    1. Conciseness reduces hallucination surface: Shorter answers have fewer
       opportunities to introduce claims not grounded in the retrieved context.
       V2's structured 3-5 sentence format provides more creative freedom,
       which can lead to extra details not supported by the context.

    2. Stronger grounding instruction: V1's "Answer using ONLY the provided
       context" is a tighter constraint than V2's step-by-step reasoning format,
       which prioritizes organization over strict context adherence.

    3. V2 leads on answer relevancy (0.871 vs 0.864) and context precision
       (0.595 vs 0.592) — its multi-step instructions tend to produce more
       comprehensive answers that better cover the question's full scope,
       and slightly better at selecting relevant context chunks.

    Recommendation: Use V1 for high-accuracy/factual use cases; use V2 when
    answer completeness and user-facing structure matter more. A hybrid prompt
    combining V1's grounding constraint with V2's structured format would likely
    achieve both high faithfulness AND high relevancy.
    """
    ...
    try:
        ...
        print(f" Warning: Could not save data/ragas_report.json: {e}")
